...
import { getPermissionsFromStorage, type UserPermissionsStorage } from "../auth/authService";
...
const DashboardApi = {
    getItems: async (body: UnknownObject) =>
        await requests.post<{ result: UnknownObject[] }>(`${ITEM}/advanceSearch/`, body),
    getPartGroups: async (body: UnknownObject) =>
        await requests.post<{ result: UnknownObject[] }>(`${PART_GROUP}/advanceSearch/`, body),
    getMpns: async (body: UnknownObject) =>
        await requests.post<{ result: UnknownObject[] }>(`${MANUFACTURER}/advanceSearch/`, body),
...
export const getConveyorBeltCards = (roles: UserPermissionsStorage["roles"]): ConveyorBeltCard[] => {
...
export const getObjectsNeedingDeclarationCards = async (): Promise<DashboardCardRow[] | undefined> => {
...
export const getPendingDataCards = async (): Promise<DashboardCardRow[] | undefined> => {
    let cards: DashboardCardRow[] | undefined;
    const permissions = getPermissionsFromStorage();
    if (permissions?.isManager || permissions?.isAdmin) {
        ...
        cards = await loadPendingRequestsCount();
    }
    return cards;
};
