...
import { type Column, tableColumnsData } from "../../../constants/tableColumns";
...
interface RequiredValidatorMessages {
    username?: Message[];
    email?: Message[];
    phone?: Message[];
    roles?: Message[];
...
export default class UserStore {
    ...
    get usersDp() {
        const copyUsers = mapRolesToName(this.users.slice());
        sortDataWithDates(copyUsers, CONSTANTS.SORT_KEY1, CONSTANTS.SORT_KEY2);
        const filterCriterion = this.filter
            ? FilterFactory.getFilter({ filterDef: { text: this.filter } })
            : undefined;
        const arrayDataProvider = new ArrayDataProvider(copyUsers, {
            keyAttributes: CONSTANTS.KEYATTR_USERS,
        });
        return new ListDataProviderView(arrayDataProvider, { filterCriterion });
    }
    ...
    private readonly validationChecksPassed = (): boolean => {
        const { username, email, roles } = this.popupUser;

        this.requiredMessageArray = {};

        const usernameErrors =
            username.length < 1
                ? [this.errorMessage(CONSTANTS.MESSAGES.FIELD_NOT_EMPTY)]
                : username.split(" ").length > 1
                ? [this.errorMessage(CONSTANTS.MESSAGES.ONLY_UNDERSCORE)]
                : !this.modeIsUpdate
                ? this.users.find((user) => user.username === username)
                    ? [this.errorMessage(CONSTANTS.MESSAGES.USERNAME_TAKEN)]
                    : []
                : [];
        ...
        const emailErrors = !email
            ...
            ? this.users.find((user) => user.email === email)
                ? [this.errorMessage(CONSTANTS.MESSAGES.EMAIL_INUSE)]
                : []
            : [];

        const rolesError =
            !roles || roles.length < 1 ? [this.errorMessage(CONSTANTS.MESSAGES.MUST_SELECT)] : [];
        const validatorMessages: RequiredValidatorMessages = {
        ...
        return !Object.values(validatorMessages)
            .flat()
            .some((value) => value);
    };

    private readonly errorMessage = (text: string) => {
        return new Message("summary", text, "error");
    };
    ...
    createOrUpdateUser = async () => {
        if (this.validationChecksPassed()) {
            ...
            try {
                const user = this.modeIsUpdate
                    ? await UserApis.updateUser(this.popupUser)
                    : await UserApis.createUser(this.popupUser);
                const filteredUsers = this.users.filter((u) => u.id !== user.id);
                this.users = [user, ...filteredUsers];
                store.commonStore.showNotification(
                    "confirmation",
                    `User ${this.modeIsUpdate ? "updated" : "created"} successfully.`,
                );
            } catch (error) {
                store.commonStore.showNotification(
                    "error",
                    `Error in ${this.modeIsUpdate ? "updating" : "creating"} user.`,
                );
            }
        }
    };
