...
import { type DashboardCardRow } from "../../../modules/dashboard/dashboardService";
import { useStore } from "../../../modules/store";
import { navigateToLink } from "../../../routes/redirection";
import MutableArrayDataProvider = require("ojs/ojmutablearraydataprovider");

export const CountCards = observer(() => {
    const userPermissions = getPermissionsFromStorage();
    const {
        dashboardStore: { cardsLoading, objectsCards, thingsCards },
    } = useStore();
...
const DashboardCard = (props: CardProps) => {
    ...
    const rowsDp = new MutableArrayDataProvider(rowCards, { keyAttributes: "label" });
    ...
    const redirect = (event: any, link: string) => {
        ...
        void navigateToLink(link);
    };

    const renderRow = (inputRow: any) => {
        if (inputRow?.item?.data) {
            const row: DashboardCardRow = inputRow.item.data;
            const linkClass = `oj-typography-subheading-md oj-link-standalone  ${row.class}`;
            return (
                <tr>
                    <td style="text-align: right; padding-right: 10px">
                        <label className={linkClass}>{row.count}</label>
                        <a className={linkClass}>{row.superscript ? <sup>+</sup> : null}</a>
                    </td>
                    <td>
                        <a
                            onClick={(event) => {
                                redirect(event, row.link);
                            }}
                            className="oj-typography-subheading-xs oj-link-standalone"
                            href={row.link}
                        >
                            {row.label}
                        </a>
    ...
    return (
        <div className="oj-flex-item oj-sm-4 oj-sm-margin-2x" style={{ cursor: "auto" }}>
            <div
                className="oj-typography-bold oj-typography-heading-xs oj-sm-margin-2x-bottom"
            ...
            <div>
                {isCardLoading ? (
                    <Skeleton
                        count={3}
                        height={35}
                        style={{ margin: "4px 0", opacity: 0.3 }}
            ...
            {!isCardLoading && (
                <span>
                    {footerText ? (
                        <oj-label class="oj-label oj-text-color-secondary">{footerText}</oj-label>
                    ) : null}
                </span>
            )}
        </div>
