...
export const ReorderColumnsPopup = (props: ReorderColumnProps) => {
    ...
    const [columnsDP, setColumnsDP] = useState<
        MutableArrayDataProvider<any, any>
    >(new MutableArrayDataProvider([], { keyAttributes: DIALOG.KEY }));
    ...
    const cancelAction = () => {
        ...
        // closeDialog(id);
        setColumnsDP(
            new MutableArrayDataProvider(columns, {
                keyAttributes: DIALOG.KEY,
            }),
        );
    };
        ...
        if (items.length > 0) {
            try {
                ...
                if (sourceInfo) {
                    ...
                    if (destInfo) {
                        if (
                            position === DIALOG.POSITION_AFTER &&
                            destInfo.index === destInfo.arr.length - 1
                        ) {
                        ...
                        setColumnsDP(
                            new MutableArrayDataProvider([...destInfo.arr], {
                                keyAttributes: DIALOG.KEY,
                            }),
                        );
                    }
    ...
    const popupBody = () => {
        return (
            <div id="container">
                <oj-label class="oj-label oj-text-color-success">
                    {DIALOG.DND}
                </oj-label>
                <h6>{DIALOG.TEXT}</h6>
